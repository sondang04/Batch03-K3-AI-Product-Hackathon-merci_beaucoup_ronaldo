...
| Lệnh | Ra gì |
|---|---|
| `/recap buoi:Day 1` | 1 message đầu (số block · độ phủ · thời gian đọc) rồi **thread**, mỗi block một message: 4-6 ý + mã đoạn + `🔑 Keyword` + `💬 Bạn học từng vướng gì ở đây`; message cuối là `❓ Chưa gán được` + nhắc giới hạn |
...
| `(không tạo được thread — gửi vào channel)` | thiếu quyền `Create Public Threads` — bot vẫn chạy, chỉ khác chỗ gửi |
